...
export async function POST(
  ...
  console.log('🔍 POST /api/gameResult/[id]/comments 시작');

  return await withSessionUser(async (user) => {
    console.log('✅ 인증 성공');

    const { id } = await params;
    console.log('📋 게임 결과 ID:', id);
...
export async function DELETE(
  ...
  return await withSessionUser(async () => {
    ...
    try {
      const gameResult = await client.getDocument(id);
      if (!gameResult) {
        return NextResponse.json(
          { error: '게임 결과를 찾을 수 없습니다.' },
          { status: 404 }
        );
      }

      const updatedComments = (gameResult.comments || []).filter(
